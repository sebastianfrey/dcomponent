define([
	...
	'dijit/form/Form',
	'dojo/dom-construct'
], function (declare, Form, domConstruct) {
    ...
    skipRendering: null,
		...
		_registerStoreEvents: function () {
			...
			if (store) {
				...
				this._storeEvents.push(store.on('add', (function (event) {
					if (!this.skipRendering && isValidEvent(store, event)) {
						this._insertItem(event.target, event.index);
					}
				}).bind(this)));
				this._storeEvents.push(store.on('update', (function (event) {
					if (!this.skipRendering && isValidEvent(store, event)) {
						this._destroyItem(event.previousIndex);
						this._insertItem(event.target, event.index);
					}
				}).bind(this)));
		...
		/**
		 * dcomponent/Component instances have to implement this method. 
		 * 
    ...
    _setSkipRenderingAttr: function(state) {
      this._set('skipRendering', state);
    }
  });
});